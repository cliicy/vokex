...
class OKCoinSpot:
    ...
    def kline(self, symbol='', since=''):
        ...
        return httpGet(self.__url, TICKER_RESOURCE, params)
    # 获取OKCOIN kline，补全信息使用信息
    def kline_bq(self, symbol='', since=''):
        TICKER_RESOURCE = "/api/v1/kline.do"
        params = ''
        if symbol:
            params = 'symbol=%(symbol)s&since=%(since)s&type=1min' % {'symbol': symbol, 'since': since}
        return httpGet(self.__url, TICKER_RESOURCE, params)


    # 获取OKCOIN kline信息
    def kline_1day(self, symbol=''):
        TICKER_RESOURCE = "/api/v1/kline.do"
        params = ''
        if symbol:
            params = 'symbol=%(symbol)s&type=1day' % {'symbol': symbol}
        return httpGet(self.__url, TICKER_RESOURCE, params)
