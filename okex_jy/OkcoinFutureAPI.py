...
class OKCoinFuture:
    ...
    def future_kline(self, symbol, contractType, since):
        ...
        params = ''
        if symbol:
            params += '&symbol=' + symbol + '&since=%(since)s&type=1min' % {
                'since': since} if params else 'symbol=' + symbol + '&since=%(since)s&type=1min' % {'since': since}
        if contractType:
            params += '&contract_type=' + contractType if params else 'contract_type=' + symbol
        return httpGet(self.__url, FUTURE_TICKER_RESOURCE, params)

    # OKex kline信息
    def future_kline_bq(self, symbol, contractType, since):
        FUTURE_TICKER_RESOURCE = "/api/v1/future_kline.do"
        params = ''
        if symbol:
            params += '&symbol=' + symbol + '&since=%(since)s&type=1min' % {
    ...
    # OKex kline信息
    def future_kline_1day(self, symbol, contractType):
        FUTURE_TICKER_RESOURCE = "/api/v1/future_kline.do"
        params = ''
        if symbol:
            params += '&symbol=' + symbol + '&type=1day' if params else 'symbol=' + symbol + '&type=1day'
        if contractType:
            params += '&contract_type=' + contractType if params else 'contract_type=' + symbol
        return httpGet(self.__url, FUTURE_TICKER_RESOURCE, params)
